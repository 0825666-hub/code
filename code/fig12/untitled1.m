...
A = 0.5;                      % Intensity of external weak signal   
w = 0.01;                     % Frequency of external weak signal    

m_period = 300;               % Number of simulation cycles  
T1 = 2 * pi * m_period / w;             
T0 = 2 * pi * 20 / w;                   
...
for d_idx = 1:num_D
    ...
    for num = 1:length(Time)
        ...
        if time > T0
            ...
            I_syn = sum(IsynE, 2).';                % Transpose
        end

        I_signal(1:TT) = A .* sin(w .* time);          % External input signal
        ...
        % Synaptic compartment calculation========================================================================
        gna = 45; Ena = 55;
        gk = 18; Ek = -80;
        gl = 0.1; El = -65;
        ...
        % Dendritic compartment calculation=====================================================================
        g_dl = 0.1; E_dl = -65;
        I_SD = gc .* (vs0 - v0) / p_;                     
        I_DL = g_dl .* (vs0 - E_dl);                     
        ...
        max_val(flag1 == 1 & v > max_val) = v(flag1 == 1 & v > max_val);
        local = find(flag1 == 1 & v < max_val);            
